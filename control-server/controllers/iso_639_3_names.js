...
const table = 'ISO_639-3_Names';

const fs = require('fs-extra');
const Model = require('../models')['ISO_639-3_Name'];
// const Op = require('../models').Sequelize.Op;

// paths
const seedPath = '../control-data/seeds/bb_nomenclature/SIL-ISO_639';
const seedFileName = 'iso-639-3_Name_Index_20190408.tab';
...
function readCSV(filePath, delimiter) {
  ...
  if (fs.existsSync(filePath)) {
    ...
    return newArray.filter(line => line).map(line => {
      ...
      return {
        Id: record[0].trim(),
        Print_Name: record[1].trim(),
        Inverted_Name: record[2].trim(),
      };
    });
  };
