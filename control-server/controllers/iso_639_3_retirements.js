...
const table = 'ISO_639-3_Retirements';

const fs = require('fs-extra');
const Model = require('../models')['ISO_639-3_Retirement'];
// const Op = require('../models').Sequelize.Op;

// paths
const seedPath = '../control-data/seeds/bb_nomenclature/SIL-ISO_639';
const seedFileName = 'iso-639-3_Retirements_20190408.tab';
...
function readCSV(filePath, delimiter) {
  ...
  if (fs.existsSync(filePath)) {
    ...
    return newArray.filter(line => line).map(line => {
      ...
      return {
        Id: record[0].trim(),
        Ref_Name: record[1].trim(),
        Ret_Reason: record[2].trim(),
        Chonge_To: record[3].trim(),
        Ret_Remedy: record[4].trim(),
        Effective: record[5].trim(),
      };
    });
  };
